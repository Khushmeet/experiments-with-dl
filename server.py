...
class http_:
    def recv_packet(self, sock, packet_size):  # Method will Analyse the HTTP Request Packet
        ...
        if file_name == ' ':  # Welcome Page of Web Server
            ...
            try:
                ...
                if os.path.isfile(file_path):
                    ...
                    sock.send("HTTP/1.1 200 Ok\n" + "Date: " + str(
                        datetime.datetime.now()) + "Server: Python Webserver/1.0" + "Content-Type: text/html" + outputdata)
                ...
                HTML = (
                    "<!DOCTYPE HTML PUBLIC -//IETF//DTD HTML 2.0//EN>"  # If file not found on Server with HTTP 404 Header.
                    + "<html>"
                    + "<head>"
        ...
        sock.send("HTTP/1.1 404 Not Found\n"
                  ...
                  + "\n");
        print
        "Response HTTP/1.1 404 ( File not found on Server )"
        return
...
class Client(http_):
    def client_(self, HOST, PORT):
        ...
        if option_selected == "1":
            ...
            file_to_access = input("Enter File Name(format filename.extension):")
            start = time.time()
            http_connection = httplib.HTTPConnection(HOST, PORT)
            http_connection.connect()
